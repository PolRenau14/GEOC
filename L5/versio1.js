...
function getFacesIncidentToPoint(p){
  ...
  console.log($.isEmptyObject(DCEL.vertices[p]))
  if ( DCEL.vertices[p] == {}) {
...
function computeTriangulation(points) {
    ...
    console.log("Despues del Find And Add")
    console.log(i)
    console.log(DCEL.vertices[i])
		var b = false;
		//while(!b)
			//b = checkDelaunay(i);
	}
  ...
  //Recalcula el index de point.
  for ( var i = 0; i < outputTriangles.length; ++i){
    for (var j = 0; j < outputTriangles[i].length;++j){
      if (outputTriangles[i][j] > 4) {
        outputTriangles[i][j] = (outputTriangles[i][j] + 1 )/2
      }
    }
  }
  console.log(DCEL)
